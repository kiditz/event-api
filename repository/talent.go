...
type FilteredTalent struct {
	...
	ExpertiseName string `query:"expertise_name"`
	Limit         int64  `query:"limit"`
	Offset        int64  `query:"offset"`
	Q             string `query:"q"`
	SubCategoryID int64  `query:"sub_category_id"`
}
...
func GetTalents(filter *FilteredTalent) []e.Talent {
	...
	query = query.Preload("Image").Offset(filter.Offset).Limit(filter.Limit).Find(&talents)
	return talents
}
