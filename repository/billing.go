...
func GetBillings(c echo.Context, filter *e.BillingFilter) []e.Billing {
	...
	if filter.StartDate != "" && filter.EndDate != "" {
		query = query.Where("due_date between to_date(?, 'YYYY-MM-DD') and to_date(?, 'YYYY-MM-DD')", filter.StartDate, filter.EndDate)
	}
	...
	query = query.Preload("Brief").Preload("Brief.PaymentTerms").Preload("Brief.PaymentDays")
	query = query.Offset(filter.Offset).Limit(filter.Limit).Order("id desc")
	query = query.Find(&billings)
	return billings
}
